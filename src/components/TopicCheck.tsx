...
function SelectionData (props) {
    ...
    const [selection,setSelection] = useState(props.selection) //list of selection
    const [source, setSource] = useState(props.source) //dictionary include line information
    const [expanded, setExpanded] = useState(props.expanded)
  
}
function TopicCheckboxTree (props) {
  // constructor(props) {
  //   super(props);
  //   this.state = {
  ...
  const [arrayIndexInputOpen, setArrayIndexInputOpen]= useState(false);
  ...
  function addExpandTopics(topic_name, topic_type, root_name, showCheckbox = true) {
  ...
  function onCheck(checked, targetNode){
  ...
  function onArrayIndexPopupClose(){
   var temp = this.state.data
  ...
  function onExpand(expanded, targetNode){
    var temp = this.state.data
        ...
        <Popup open={this.state.arrayIndexInputOpen}
                ...
                <form onSubmit={this.onArrayIndexAdd}>
                  ...
                  <input type="number" 
                    ...
                    min='0'
                    onChange={ (e)=> this.setState({arrayIndex: Math.round(Number(e.target.value))})} 
                  />
